...
def real_time(time_real):
    return time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(time_real))
def drag_file():
    print("===正在检测是否有拖入的文件===")
    try:
        calculate_hash_drag_file_path = sys.argv[1]
        calculate_hash_drag_file_path_info = os.stat(calculate_hash_drag_file_path)
        
        print("===正在尝试输出文件信息===")
        print("文件索引号：",calculate_hash_drag_file_path_info.st_ino)
        print("文件被连接数目：",calculate_hash_drag_file_path_info.st_dev)
        print("文件文件大小:",calculate_hash_drag_file_path_info.st_size,"字节")
        print("最后一次访问时间:",real_time(calculate_hash_drag_file_path_info.st_atime))
        print("最后一次修改时间:",real_time(calculate_hash_drag_file_path_info.st_mtime))
        print("最后一次状态变化的时间：",real_time(calculate_hash_drag_file_path_info.st_ctime))
        calculate_hash(calculate_hash_drag_file_path)
        print("===输出结束===")
    except Exception as hash_input_Error:
        print("Debug:",hash_input_Error)
        print("===输出结束===")
        
def calculate_hash(calculate_hash_drag_file_path_info):
    try:
        print("拖入的文件路径:", calculate_hash_drag_file_path_info)
        print("SHA-256:",calculate_sha256(calculate_hash_drag_file_path_info))
    except Exception as calculate_hash_Error:
        print(calculate_hash_Error)

def calculate_sha256(file_path):
    sha256_hash = hashlib.sha256()
    
    # 以二进制模式读取文件，分块处理以避免大文件内存问题
...
def upgrade_check():
    print("Hyper Tools Kits 1.0.04 当前为预览版,可能存在bug")
    ...
    except Exception as check_updates_Error:
        print("Error:",check_updates_Error)
...
================================     
        ...
        if get_command=="1":
            ...
            hosts_2 = str(input("main/hosts/>>> 请输入要执行的域名(选择4请忽略):"))
            hosts_3 = str(input("main/hosts/>>> 请输入要执行的域名ip地址(选择2和4请忽略):"))
